...
A focused, minimal AI ecosystem configurator **for Claude Code only**.

## What it does

`system-general-ai` configures Claude Code with:

- A custom **system prompt** (output style) — short, direct, no pedagogical voice, neutral Spanish + English
- **Spec-Driven Development** workflow — orchestrator + 9 phase skills with per-phase model assignment (Opus for architectural decisions, Sonnet for execution, Haiku for cheap tasks)
...
- **Multi-Claude support** — auto-discovers every `~/.claude*` install on the machine and syncs all of them
...
If you use Claude Code daily and want the productivity layer without:

- A chatty pedagogical persona that adds tokens to every turn
- An installer that requires cloning a repo, aliasing binaries, and migrating configs across multiple Claude credentials
- Adapters for agents you don't use (Cursor, Gemini, etc.)
- Components you'll never touch

…then this is the wrapper for you. It does one thing — configure Claude Code well — and gets out of the way.
...
5. Run `system-general-ai install` automatically (zero questions, smart defaults)
...
internal/claude/             Multi-Claude detection + sync to settings/CLAUDE.md
